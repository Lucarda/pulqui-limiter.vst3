...
tresult PLUGIN_API PlugController::initialize (FUnknown* context)
{
    ...
    if (result == kResultTrue)
    {
        ...
        snprintf(text, 32, "Split C-D");
        auto* splitParam3 = new SplitParameter (Vst::ParameterInfo::kCanAutomate,
            PulquiCrossoverParams::kParamSplit_3_Id, text);
        parameters.addParameter (splitParam3);
...
tresult PLUGIN_API PlugController::setComponentState (IBStream* state)
{
    ...
    if (streamer.readFloat (Split_3) == false)
        return kResultFalse;
    setParamNormalized (PulquiCrossoverParams::kParamSplit_3_Id, Split_3);

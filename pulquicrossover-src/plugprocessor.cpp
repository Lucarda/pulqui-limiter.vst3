    ...
    if (data.inputParameterChanges)
    {
        ...
        for (int32 index = 0; index < numParamsChanged; index++)
        {
                    ...
                    data.inputParameterChanges->getParameterData (index))
            {
                ...
                {
                    case PulquiCrossoverParams::kParamSplitId:
                    ...
                    case PulquiCrossoverParams::kParamLpId:
                        if (paramQueue->getPoint (numPoints - 1, sampleOffset, value) ==
                            kResultTrue)
                            mLp = (value > 0.5f);
                        break;
...
tresult PLUGIN_API PlugProcessor::getState (IBStream* state)
{
    ...
    float savedParam1 = mSplitValue;
    int32 BypassState = mBypass ? 1 : 0;
    int32 LpState = mLp ? 1 : 0;

    IBStreamer streamer (state, kLittleEndian);
    streamer.writeFloat (savedParam1);
    streamer.writeInt32 (BypassState);
    streamer.writeInt32 (LpState);
